...
type HelmfileExecutor interface {
	...
	Apply(ctx context.Context, options app.ConfigProvider, applyOptions app.ApplyConfigProvider) (string, error)

	// Diff runs helmfile diff to show changes
	Diff(ctx context.Context, options app.ConfigProvider, diffOptions app.DiffConfigProvider) (string, error)

	// Template runs helmfile template to render manifests
	Template(ctx context.Context, options app.ConfigProvider, templateOptions app.TemplateConfigProvider) (string, error)

	// Destroy runs helmfile destroy to delete releases
	Destroy(ctx context.Context, options app.ConfigProvider, destroyOptions app.DestroyConfigProvider) (string, error)
